...
func main() {
	...
	//绑定静态资源入口
	router.PathPrefix("/dist/").Handler(http.FileServer(http.Dir("./public")))
	router.PathPrefix("/AdminLTE/").Handler(http.FileServer(http.Dir("./public")))
	router.PathPrefix("/plugins/").Handler(http.FileServer(http.Dir("./public")))
